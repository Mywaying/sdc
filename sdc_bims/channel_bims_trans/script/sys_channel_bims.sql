use ods;
drop table if exists  odstmp_bims_biz_channel_common;
CREATE TABLE ods.odstmp_bims_biz_channel_common ( id STRING COMMENT 'ID',
...
insert overwrite table odstmp_bims_biz_channel_common SELECT
bc.*,
...
FROM
  ods_bims_biz_channel bc
left join (select DISTINCT KEY_,NAME_new_ NAME_ from ods_bims_sys_dic_common where TYPE_ID_='10000026330035' or TYPE_ID_='10000026340035'  )d on d.KEY_=lower(bc.`channel_type`);
drop table if exists ods_bims_biz_channel_common;
ALTER TABLE odstmp_bims_biz_channel_common RENAME TO ods_bims_biz_channel_common;