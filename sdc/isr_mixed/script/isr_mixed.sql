use ods;
drop table if  exists ods.odstmp_bpms_biz_isr_mixed_common;
CREATE TABLE ods.odstmp_bpms_biz_isr_mixed_common (   id STRING COMMENT '主键id',
  apply_no STRING COMMENT '订单编号',
  contract_no STRING,
  is_need_mortgage STRING COMMENT '是否需我司办理按揭',
  tail_release_node STRING COMMENT '尾款释放',
  tail_release_node_name STRING,
  ...
  risk_level_name STRING COMMENT '风险等级名',
  ...
  is_automated_approve_done STRING COMMENT '判断资料外传节点的自动审批是否做了，使用0,1来判断',
  ...
  moo_inputcomple_time timestamp COMMENT '通知录入完成时间',
  ...
  is_send_cust_sign STRING COMMENT '是否发送客户签约合同成功（1成功，2失败，3合同撤销失败，null 未处理）',
  product_id STRING COMMENT '产品ID',
  product_name STRING COMMENT '产品名字',
  is_special_approved string COMMENT '是否特批业务（1-是，非1-否）',
  feedback_on_abnormal_order_flag string COMMENT '是否特批业务（1-是，非1-否）',
  loan_way string COMMENT '放款方式',
  cooperative_guarantee_agency string COMMENT '合作担保机构'
)  STORED AS parquet;

insert overwrite table odstmp_bpms_biz_isr_mixed_common
select
a.id,
a.apply_no,
a.contract_no,
a.is_need_mortgage,
a.tail_release_node,
nvl(nvl(bp.loan_node_name,b.NAME_),a.tail_release_node) tail_release_node_name,
a.charging_time,
...
bao.product_id,
bao.product_name,
a.is_special_approved,
a.feedback_on_abnormal_order_flag,
a.loan_way,
a.cooperative_guarantee_agency
FROM ods_bpms_biz_isr_mixed a
left join (select apply_no,product_id,product_name from ods_bpms_biz_apply_order_common) bao on a.apply_no=bao.apply_no
left join (select KEY_,NAME_ from
(select b.*,
row_number() over(partition by b.KEY_ order by KEY_) rn
from (select DISTINCT KEY_,NAME_new_ NAME_ from ods_bpms_sys_dic_common where type_id_='10000022881903' or type_id_='10000039360006' or type_id_='10000039360008' or type_id_='10000039360009' or type_id_='10000039360010' or type_id_='10000047750219' or type_id_='10000026360013' or type_id_='10000027530016' or type_id_='10000027530016' or type_id_='10000027530011' or type_id_='10000027530018' or type_id_='10000027530014' or type_id_='10000047750219' or type_id_='10000096080009' or type_id_='10000026360013')b
)T where rn=1)b
on b.KEY_=lower(a.tail_release_node)
left join (select distinct product_id,loan_node_code,loan_node_name from ods.ods_bpms_b_product_node) bp on  bp.loan_node_code=a.tail_release_node
and bp.product_id=bao.product_id;
drop table if exists ods_bpms_biz_isr_mixed_common;
ALTER TABLE odstmp_bpms_biz_isr_mixed_common RENAME TO ods_bpms_biz_isr_mixed_common;