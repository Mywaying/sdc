...
O_SOURCE_DB=`cat $CFG | grep O_SOURCE_DB | awk -F'=' '{ print $2 }' | sed s/[[:space:]]//g`
...
O_SOURCE_DB=`cat $ODS_CFG | grep "^$O_SOURCE_DB" | awk -F'=' '{ print $2 }' | sed s/[[:space:]]//g`
...
echo "/usr/bin/python $GENSQL_CMD --host=$HOST --port=$PORT --user=$USER --password=$PASSWORD --db=$O_SOURCE_DB --predb=$SOURCE_DB --tables=$TAB_NAME --prefix=$PREFIX" |tee -a $TMPLOG
rm -rvf /tmp/ods/${SOURCE_DB}_${TAB_NAME}hcols.txt
rm -rvf /tmp/ods/${SOURCE_DB}_${TAB_NAME}hsql.txt
a=`/usr/bin/python $GENSQL_CMD --host=$HOST --port=$PORT --user=$USER --password=$PASSWORD --db=$O_SOURCE_DB --predb=$SOURCE_DB  --tables=$TAB_NAME --prefix=$PREFIX`
tabcol=`cat /tmp/ods/${SOURCE_DB}_${TAB_NAME}hcols.txt`
tabhsql=`cat /tmp/ods/${SOURCE_DB}_${TAB_NAME}hsql.txt`
echo "$tabhsql" |tee -a $TMPLOG
touch /tmp/${O_SOURCE_DB}_${TAB_NAME}.pid
mkdir -pv /tmp/log
KETTLE_LOG=/tmp/log/${TAB_NAME}.log
NUM=20
while [ -f /tmp/${O_SOURCE_DB}_${TAB_NAME}.pid -a ${NUM} -gt 0 ];
do
    ...
    sh ${V_KETTLE_DIR}/kitchen.sh -rep $REP -user admin -pass admin -dir /  /job ${JOB_FILE} -param:datestr=${SUFFIX} -param:orcdb=${O_SOURCE_DB} -param:srcdb=${SOURCE_DB} -param:tabnn=${TAB_NAME} -param:tabcol="${tabcol}"  -param:tabhsql="${tabhsql}"  2>&1| tee -a $KETTLE_LOG
    if grep -q "Finished with error" $KETTLE_LOG; then
        ...
        rm -rvf /tmp/${O_SOURCE_DB}_${TAB_NAME}.pid
        echo "">$KETTLE_LOG
    fi
    NUM=`expr $NUM - 1`
done
