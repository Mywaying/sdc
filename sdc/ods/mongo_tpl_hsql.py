...
def get_table_info(host,user,password,schema,port,table,ispartition=False,prefix=''):
    '''
    ...
    cols = []
    create_head = '''
create table if not exists {0}.{1}('''.format(prefix, prefix+"_"+schema+"_"+table)
    if ispartition:
        create_tail = r'''
partitioned by(inc_day string)
row format delimited fields terminated by '\t'
location '/user/hive/warehouse/{0}';'''.format(prefix+"_"+schema+"_"+table)
    else:
        create_tail = r'''
row format delimited fields terminated by '\t'
location '/user/hive/warehouse/{0}';'''.format(prefix+"_"+schema+"_"+table)
...
def create_sql(options):
    ...
    for tbname in tables[0].split(','):
        cols, str1 = get_table_info(host, user, password, dbnanme, port, tbname, ispartition, prefix)
        # f1 = open(u'F:\\DDJF\\xys_dbm\\kettle\\'+dbnanme+'_'+tbname+'hcols.txt','w')
        # f2 = open(u'F:\\DDJF\\xys_dbm\\kettle\\'+dbnanme+'_'+tbname+'hsql.txt', 'w')
        f1 = open(u'/tmp/ods/'+dbnanme+"_"+tbname+'hcols.txt', 'w')
        f2 = open(u'/tmp/ods/'+dbnanme+"_"+tbname+'hsql.txt', 'w')
