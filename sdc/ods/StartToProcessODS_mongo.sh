...
TMPLOG=/tmp/StartToProcessODS_mongo.log
...
do
      ...
      echo "sh ${V_SH_DIR}/start_hive_mongo_job.sh ${tab_name} ${V_JOB_FILE} ${V_TABLE_FILE} ${SUFFIX}>${V_LOG_DIR}/${tab_name}.log`date "+%F"` 2>&1" |tee -a $TMPLOG
      sh ${V_SH_DIR}/start_hive_mongo_job.sh ${tab_name} ${V_JOB_FILE} ${V_TABLE_FILE} ${SUFFIX}>${V_LOG_DIR}/${tab_name}.log`date "+%F"` 2>&1
