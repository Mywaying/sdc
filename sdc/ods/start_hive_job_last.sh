...
if grep -q "Finished with error" $KETTLE_LOG; then
    echo "TABLE ${TAB_NAME} FROM ${SOURCE_DB} TO ODS FAILED AT ${TIME}" >>${V_LOG_DIR}/ODS_ERROR_LAST`date +%F`.txt
    echo "TABLE ${TAB_NAME} FROM ${SOURCE_DB} TO ODS FAILED at ${TIME}" |mail -a $TMPLOG -s "[FAILED]TABLE ${TAB_NAME} FROM ${SOURCE_DB} TO ODS" $MAILS
    if [ $? -ne 0 ];then
