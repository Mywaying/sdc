use ods;
drop table if exists  odstmp_bpms_bpm_check_opinion_common_2;
CREATE TABLE if not exists ods.odstmp_bpms_bpm_check_opinion_common_2 (
  apply_no STRING,
  product_id STRING,
...
drop table if exists odstmp_bpms_bpm_check_opinion_common_2_complete;
insert overwrite table  odstmp_bpms_bpm_check_opinion_common_2
SELECT
bao.apply_no,
bao.product_id,
	CASE
...
when LOWER(task_key_)='agreeloanresult' and task_name_='确认银行放款' then '确认银行放款'
...
FROM
(select * from ods_bpms_bpm_check_opinion where SUP_INST_ID_<>'0' and SUP_INST_ID_ <>'') a
left join ods_bpms_biz_apply_order bao on (a.SUP_INST_ID_= bao.flow_instance_id )
LEFT JOIN ods_bpms_bpm_pro_inst t2 ON bao.flow_instance_id = t2.ID_;
drop table if exists ods_bpms_bpm_check_opinion_common_2;
ALTER TABLE odstmp_bpms_bpm_check_opinion_common_2 RENAME TO ods_bpms_bpm_check_opinion_common_2;