use ods;
CREATE TABLE if not exists ods.odstmp_bpms_order_fees_common_wx (
  apply_no STRING,
  ...
  financingcostrateperday_fee_value DOUBLE comment '融资成本率/天',
  guaranteefeerateperday_fee_value DOUBLE comment '担保费/天'
) STORED AS parquet;

insert overwrite table  odstmp_bpms_order_fees_common_wx
select t.`apply_no`,
...
max((case when lower(t.`fee_define_no`)='financingcostrateperday' then t.`fee_value` else null end)) financingcostrateperday_fee_value,
max((case when lower(t.`fee_define_no`)='guaranteefeerateperday' then t.`fee_value` else null end)) guaranteefeerateperday_fee_value
from ods_bpms_biz_fee_detial t group by t.`apply_no`;
drop table if exists ods_bpms_order_fees_common_wx;
ALTER TABLE odstmp_bpms_order_fees_common_wx RENAME TO ods_bpms_order_fees_common_wx;