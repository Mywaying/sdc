use ods;
insert overwrite table ods_rcs_icrPersonRecordDetail
  select *
  from (SELECT
        ...
        from
          ods_rcs_uat_credit_icr_report LATERAL VIEW  explode(split(regexp_replace(regexp_replace(get_json_object(`icrcreditdto`, '$.icrPersonRecordDetail'), '\\[\\{', ''), '}]', ''), '},\\{')) addTable as mvtstr) T
